...
from src.connectors.confluence.impl import ConfluenceConnector
...
@router.get(
    ...
    summary="Confluence connection status",
    description="Returns connection status for the Confluence connector for the given tenant.",
)
...
@router.get(
    ...
    description="Search resources on Confluence returning normalized items (stubbed without live token).",
)
def search(
    request: Request,
...
@router.post(
    ...
    description="Create a Confluence resource using a generic payload and return a normalized result (requires connection).",
)
def create_resource(
    request: Request,
...
@router.get(
    "/oauth/login",
    summary="Start Confluence OAuth",
...
@router.get(
    "/oauth/callback",
    response_model=ConnectionStatus,
    summary="Confluence OAuth callback (connector)",
...
@router.get(
    "/resource/{key}",
    summary="Get Confluence resource by key",
    description="Fetch a Confluence resource by key or id. Stubbed in scaffolding phase.",
)
def get_resource(
